...
-(void)handleBelowEqualIos15WithOrientationMask:(NSInteger) orientationMask viewController: (CDVViewController*) vc result:(NSMutableArray*) result selector:(SEL) selector
{
    ...
    if (orientationMask != 15) {
        ...
        // Unlocking - restore last orientation if available
        ...
        // Only call the old selector if it exists (for backward compatibility)
...
-(void)handleAboveEqualIos16WithOrientationMask:(NSInteger) orientationMask viewController: (CDVViewController*) vc result:(NSMutableArray*) result selector:(SEL) selector
{
    ...
    if (orientationMask != 15) {
        ...
        // Unlocking - only call the old selector if it exists (for backward compatibility)
        if ([vc respondsToSelector:selector]) {
            ((void (*)(CDVViewController*, SEL, NSMutableArray*))objc_msgSend)(vc,selector,result);
        }
    }
...
-(void)screenOrientation:(CDVInvokedUrlCommand *)command
{
    ...
    // Convert bit mask to orientation mask for iOS
    UIInterfaceOrientationMask iOSOrientationMask = 0;
    if(orientationMask & 1) {
        [result addObject:[NSNumber numberWithInt:UIInterfaceOrientationPortrait]];
        iOSOrientationMask |= UIInterfaceOrientationMaskPortrait;
    }
    if(orientationMask & 2) {
        [result addObject:[NSNumber numberWithInt:UIInterfaceOrientationPortraitUpsideDown]];
        iOSOrientationMask |= UIInterfaceOrientationMaskPortraitUpsideDown;
    }
    if(orientationMask & 4) {
        [result addObject:[NSNumber numberWithInt:UIInterfaceOrientationLandscapeRight]];
        iOSOrientationMask |= UIInterfaceOrientationMaskLandscapeRight;
    }
    if(orientationMask & 8) {
        [result addObject:[NSNumber numberWithInt:UIInterfaceOrientationLandscapeLeft]];
        iOSOrientationMask |= UIInterfaceOrientationMaskLandscapeLeft;
    }
    
    // Store the orientation mask for use in supportedInterfaceOrientations
    if (orientationMask == 15) {
        _supportedOrientationMask = UIInterfaceOrientationMaskAll;
    } else {
        _supportedOrientationMask = iOSOrientationMask;
    }
    
    SEL selector = NSSelectorFromString(@"setSupportedOrientations:");
    
    // Try to set supported orientations if the selector exists (older Cordova versions)
    if([vc respondsToSelector:selector]) {
    ...
    // Handle orientation change for both old and new Cordova versions
    if ([UIDevice currentDevice] != nil){
        [self handleWithOrientationMask:orientationMask viewController:vc result:result selector:selector];
    }
    
    // Force the view controller to update its supported orientations
    if (@available(iOS 16.0, *)) {
        [vc setNeedsUpdateOfSupportedInterfaceOrientations];
    } else {
        // For iOS 15 and below, attempt rotation
        [UIViewController attemptRotationToDeviceOrientation];
    }
...
// Override supportedInterfaceOrientations to return our custom mask
